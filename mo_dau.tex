\chapter{I. GIỚI THIỆU TỔNG QUAN}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Attention-weighted Merging}: ưu tiên gộp các token có trọng số chú ý thấp, đồng thời bảo vệ các token thuộc vị trí khía cạnh (được đánh dấu bởi LCF) cũng như các token có trọng số chú ý cao nhằm hạn chế mất mát thông tin ngữ nghĩa quan trọng.
    \end{itemize}
...
\begin{itemize}
    ...
    \item Ba chiến lược gộp token khác nhau, bao gồm \textit{bipartite matching}, \textit{Sequential Local Merging} và \textit{Attention-weighted Merging}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{RQ2}: Các chiến lược gộp token khác nhau (\textit{Bipartite Matching}, \textit{Sequential Local Merging} và \textit{Attention-weighted Merging}) ảnh hưởng như thế nào đến độ chính xác của mô hình trên hai nhiệm vụ dự đoán cảm xúc và nhóm khía cạnh?
    ...
    \item \textbf{RQ4}: Chiến lược \textit{Attention-weighted Merging} có đạt được sự cân bằng tốt hơn giữa hiệu năng tính toán và độ chính xác so với chiến lược ToMe gốc hay không?
\end{itemize}
...
\begin{itemize}
  \item Văn bản đánh giá (review) của khách hàng đối với dịch vụ khách sạn, viết bằng tiếng Anh lẫn tiếng Việt, được gán nhãn theo định dạng \textit{aspect term, aspect category, sentiment}.
  ...
  \item Kỹ thuật Token Merging (ToMe) và các chiến lược lựa chọn cặp token để gộp dựa trên \textit{cosine similarity} và/hoặc \textit{attention weight}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Các thực nghiệm chỉ khảo sát ba chiến lược Token Merging gồm \textit{Bipartite Matching}, \textit{Sequential Local Merging} và \textit{Attention-weighted Merging}.
